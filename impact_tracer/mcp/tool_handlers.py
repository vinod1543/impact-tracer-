...
from impact_tracer.core.git_diff import (
    get_branch_diff,
    get_staged_diff,
    get_unstaged_diff,
)
...
def analyze_git_change(
    ...
    """Analyze changes pulled directly from git.

    Args:
        project_path: Path to the git repository.
        source: One of 'unstaged', 'staged', or 'branch'.
        base_branch: Base branch for comparison (only for source='branch').

    Returns:
        dict[str, object]: Report metadata, report id, and git source info.
    """
    if source == "staged":
        gc = get_staged_diff(project_path)
    elif source == "branch":
        gc = get_branch_diff(project_path, base=base_branch)
    else:
        gc = get_unstaged_diff(project_path)

    if gc is None:
        return {"error": f"No {source} Python changes detected in {project_path}"}

    report = analyze(gc.diff_text, project_path)
    report_id = f"rep-{len(_REPORT_STORE) + 1}"
    _REPORT_STORE[report_id] = report
    return {
        "report_id": report_id,
...
def query_dependency_graph(
    project_path: str, symbol_id: str | None = None
) -> dict[str, object]:
...
def get_risk_score(
    diff_text: str, project_path: str, symbol_id: str
) -> dict[str, object]:
